...
func NewElector(cfg ElectorConfig) *Elector {
	...
	return &Elector{
		...
		electWithEpoch:   rueidis.NewLuaScript(store.LuaLeaderElectWithEpochScript()),
		refreshWithEpoch: rueidis.NewLuaScript(store.LuaLeaderRefreshWithEpochScript()),
...
// tryCampaign attempts to become the leader using atomic SET NX + epoch increment.
func (e *Elector) tryCampaign() bool {
	key := e.leaderKeyName()
	epochKey := store.ElectionEpochKey(e.prefix)
	ttlSec := strconv.Itoa(int(e.ttl.Seconds()))

	newEpoch, err := e.electWithEpoch.Exec(e.ctx, e.rdb,
		[]string{key, epochKey},
		[]string{e.nodeID, ttlSec},
	).AsInt64()
	if err != nil {
		e.logger.Warn().Err(err).Msg("election: elect with epoch failed")
		return false
	}
	if newEpoch == 0 {
...
func (e *Elector) refreshLoop() {
	...
	for {
		...
		case <-ticker.C:
			ttlSec := strconv.Itoa(int(e.ttl.Seconds()))
			result, err := e.refreshWithEpoch.Exec(e.ctx, e.rdb,
				[]string{e.leaderKeyName(), epochKey},
				[]string{e.nodeID, ttlSec},
			).AsInt64()
			if err != nil {
			...
			// Update epoch (may have changed if there was a brief hiccup).
			e.mu.Lock()
			e.epoch = uint64(result)
			e.mu.Unlock()
		}
	}
}
