...
func (s *Server) onElected() {
	s.logger.Info().Msg("this node is now the leader — starting scheduler and orchestrator")
	s.sched.Start(s.ctx)
	s.orch.Start(s.ctx)
}
...
func (s *Server) onDemoted() {
	s.logger.Info().Msg("this node lost leadership — stopping scheduler and orchestrator")
	s.sched.Stop()
	s.orch.Stop()
}
